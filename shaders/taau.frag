...
void main() {
    ...
    // Sky early-out — sky pixels are procedurally generated by compose
    // and have no useful temporal signal. Blending a previous-frame
    // castle/mountain pixel into a sky pixel via a too-loose variance
    // clip at the silhouette is what produces the "ghost castle in
    // the sky right of it" artifact at non-100% render scale.
    float d_lr = texelFetch(depth_lr, lr_ip, 0).r;
    if (d_lr >= 0.99999) {
        outColor = vec4(texture(cur_lr, lr_uv).rgb, 1.0);
        return;
    }
